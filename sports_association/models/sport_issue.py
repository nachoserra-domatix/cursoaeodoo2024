...
class SportIssue(models.Model):
    ...
    state = fields.Selection(
        selection=SPORT_ISSUE_STATE, string="Status", default="draft", tracking=True
    )
    user_id = fields.Many2one(
        comodel_name="res.users", string="User", default=lambda self: self.env.user
    )
    sequence = fields.Integer(string="Sequence", default=10)
    solution = fields.Html(string="Solution")

    clinic_id = fields.Many2one(comodel_name="sport.clinic", string="Clinic")
    tag_ids = fields.Many2many(comodel_name="sport.issue.tag", string="Tag")
    cost = fields.Integer(string="Cost")
    color = fields.Integer(string="Color")
    assigned = fields.Boolean("Assigned", compute="_compute_assigned", store=True)
    action_ids = fields.One2many(
        "sport.issue.action", "issue_id", string="Actions to do"
    )
